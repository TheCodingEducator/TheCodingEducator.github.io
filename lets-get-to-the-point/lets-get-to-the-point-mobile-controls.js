...
(function () {
  ...
  style.textContent =
    ...
    '#mobile-controls { position: fixed; left: 0; right: 0; bottom: 0; height: 190px; z-index: 10001; pointer-events: none; touch-action: none; user-select: none; -webkit-user-select: none; -webkit-touch-callout: none; }' +
    // pointer-events starts at none, not auto - an always-on "auto"
    // here would permanently swallow every tap/scroll in this
    // full-width bottom strip even on the start/skill-select/results
    // screens, where the joystick isn't needed and the standards panel/
    // teaching notes further down the page can scroll into this same
    // band. .mc-active (toggled in refreshVisibility below) re-enables
    // it only while the player is actually moving the point.
  ...
  // ---- Show the joystick/action bar only while the point is actually
  // being moved - not on the start/skill-select/feedback/results
  // screens, where the joystick does nothing and this full-width strip
  // would otherwise block reaching the standards panel/teaching notes
  // below the fold. ----
  function refreshVisibility() {
    var show = typeof STATE !== 'undefined' && (STATE === 'MOVING' || STATE === 'SHOWING');
